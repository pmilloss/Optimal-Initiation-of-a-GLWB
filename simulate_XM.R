simulate_XM <- function(nsim, X0 = 1, M0 = X0, dS_S, N, d, psi, phi, beta)
{
  ...
  if((nrow(dS_S) != N) || (ncol(dS_S) != nsim)) warning("dS_S must be a matrix with appropriate dimensions")
...
#   # phi: the insurance fee
#   # beta: the roll-up rate
#   # gt: the annuity rate if GMWB is initiated at t, a scalar
...
calculate_F <- function(nsim, Xt, Mt, dS_S, N, d, psi, phi, beta = 0, gt, tau, B)
{
